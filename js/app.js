    ...
    this.$get = function() {
        ...
        return {
            selectTemplate: function (device) {
                ...
                if(device == null || device.functions == undefined || device.variables == undefined) {
                    return template;
                }
                for (var appIndex = 0; appIndex < currentApps.length; appIndex++) {
                    var appMatches = true;
                    var app = currentApps[appIndex];
                    for(var reqFuncIndex = 0; reqFuncIndex < app.required.functions.length; reqFuncIndex++) {
                        var requiredFunction = app.required.functions[reqFuncIndex];
                        if (device.functions.indexOf(requiredFunction) < 0) {
                            appMatches = false;
                            break;
                        }
                    }
                    for(var reqVarIndex = 0; reqVarIndex < app.required.variables.length; reqVarIndex++) {
                        var requiredVariable = app.required.variables[reqVarIndex];
                        if (device.variables[requiredVariable] === undefined) {
                            appMatches = false;
                            break;
                        }
                    }
